...
"""
    ...
    def analyze_community(self, collected_data: str, previous_titles: list = None) -> tuple:
        """커뮤니티 핫토픽 리포트 생성. (title, keywords, report) 튜플 반환"""
        now_kst = datetime.now(self.kst)
        timestamp = now_kst.strftime("%Y-%m-%d %H:%M KST")

        prompt = f"""당신은 온라인 커뮤니티 트렌드 분석 전문가입니다. 아래 수집된 데이터에서 **한국과 일본 온라인 커뮤니티의 핫토픽**을 분석하여 한국어로 리포트를 작성해주세요.

**중요: 리포트 맨 첫 줄에 반드시 아래 형식으로 제목과 키워드를 작성하세요:**
TITLE: [오늘 커뮤니티에서 가장 핫한 주제 한 줄 요약 (15자 이내)]
KEYWORDS: [핵심 키워드 2-3개, 쉼표로 구분]

예시:
TITLE: 연말 보너스 논쟁 폭발
KEYWORDS: 연말보너스, 직장인, 회사

TITLE: 일본 신작 애니 화제
KEYWORDS: 애니메이션, 일본, 신작

TITLE: 게임 업데이트 유저 분노
KEYWORDS: 게임, 업데이트, 유저반응
...
**추가 지침:**
- 민감한 정치/혐오 발언은 중립적으로 요약하거나 생략
- 커뮤니티 특유의 은어나 밈은 이해하기 쉽게 설명
- 5ch 일본어 게시글은 한국어로 번역하여 요약

## 리포트 형식 (아래 형식을 정확히 따라주세요)

## 1. 오늘의 핫토픽 TOP 5

**커뮤니티 전체에서 가장 뜨거운 주제 5개를 선정하세요:**

### 1. [주제]
• 출처: [커뮤니티명]
• 요약: [무슨 일인지 2-3문장으로 설명]
• 반응: [커뮤니티 반응/여론]
...
### 5ch (일본)
• [주요 화제 1-2개 간단 요약 - 한국어로 번역]


## 3. 밈 & 유행어

**오늘 커뮤니티에서 유행하는 밈이나 신조어가 있다면 소개:**
...
[오늘 커뮤니티 여론의 전반적인 분위기와 트렌드를 2-3문장으로 요약]

---
커뮤니티 게시글의 핵심만 추출하여 읽기 쉽게 정리하세요.
"""
